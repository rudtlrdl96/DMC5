...
{
	if (nullptr == GameEngineFBXMesh::Find("em0102.FBX"))
	{
	...
	{
		EnemyRenderer->SetFBXMesh("em0102.fbx", "AniFBX_Low");
	}
	break;
	case GameEngineOptionValue::High:
	{
		EnemyRenderer->SetFBXMesh("em0102.fbx", "AniFBX");
	}
	break;
	default:
		break;
...
void Enemy_RedEmpusa::Start()
{
	...
	// 넷 오브젝트 타입 설정
	SetNetObjectType(Net_ActorType::RedEmpusa);
...
void Enemy_RedEmpusa::DeathCheck()
{
	if (EnemyHP <= 0)
	{
		DeathValue = true;
...
void Enemy_RedEmpusa::PlayerChase(float _DeltaTime)
{
	RotationCheck();
...
void Enemy_RedEmpusa::PlayerAttack(float _DeltaTime)
{
	RotationCheck();
	AllDirectSetting();
...
void Enemy_RedEmpusa::RandomAttack()
{
	int RandC = GameEngineRandom::MainRandom.RandomInt(0, 2);
...
void Enemy_RedEmpusa::AttackCalculation()
{
	AttackDirectCheck();
...
void Enemy_RedEmpusa::DamageCollisionCheck(float _DeltaTime)
{
	if (true == DeathValue)
	{
		return;
...
void Enemy_RedEmpusa::DamageCollisionCheck_Client(float _DeltaTime)
{
	if (true == DeathValue)
	{
		return;
...
void Enemy_RedEmpusa::RecognizeCollisionCheck(float _DeltaTime)
{
...
void Enemy_RedEmpusa::ChangeState(int _StateValue)
{
...
void Enemy_RedEmpusa::ChangeState_Client(int _StateValue)
{
	EnemyFSM.ChangeState(_StateValue);
	EnemyFSMValue = _StateValue;
...
void Enemy_RedEmpusa::EnemyCreateFSM()
{
	...
	EnemyFSM.ChangeState(FSM_State_RedEmpusa::RedEmpusa_Idle);
}
