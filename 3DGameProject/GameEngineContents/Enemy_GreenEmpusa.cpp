...
{
	EnemyCodeValue = EnemyCode::GreenEmpusa;
	EnemyHP = 1500;
}

void Enemy_GreenEmpusa::EnemyMeshLoad()
{
	if (nullptr == GameEngineFBXMesh::Find("em0101.FBX"))
	{
		std::string Path = GameEnginePath::GetFileFullPath("ContentResources",
			{
				"Character", "Enemy", "GreenEmpusa", "mesh"
			},
			"em0101.FBX");
	...
	{
		EnemyRenderer->SetFBXMesh("em0101.fbx", "AniFBX_Low");
	}
	break;
	case GameEngineOptionValue::High:
	{
		EnemyRenderer->SetFBXMesh("em0101.fbx", "AniFBX");
	}
	break;
	default:
		break;
...
void Enemy_GreenEmpusa::DeathCheck()
{
	if (EnemyHP <= 0)
	{
		DeathValue = true;
...
void Enemy_GreenEmpusa::PlayerChase(float _DeltaTime)
{
	RotationCheck();
...
void Enemy_GreenEmpusa::PlayerAttack(float _DeltaTime)
{
	RotationCheck();
	AllDirectSetting();
...
void Enemy_GreenEmpusa::RandomAttack()
{
	int RandC = GameEngineRandom::MainRandom.RandomInt(0, 2);
...
void Enemy_GreenEmpusa::AttackCalculation()
{
	AttackDirectCheck();
...
void Enemy_GreenEmpusa::DamageCollisionCheck(float _DeltaTime)
{
	if (true == DeathValue)
	{
		return;
...
void Enemy_GreenEmpusa::DamageCollisionCheck_Client(float _DeltaTime)
{
	if (true == DeathValue)
	{
		return;
...
void Enemy_GreenEmpusa::RecognizeCollisionCheck(float _DeltaTime)
{
...
void Enemy_GreenEmpusa::ChangeState(int _StateValue)
{
...
void Enemy_GreenEmpusa::ChangeState_Client(int _StateValue)
{
	EnemyFSM.ChangeState(_StateValue);
	EnemyFSMValue = _StateValue;
...
void Enemy_GreenEmpusa::EnemyCreateFSM()
{
	...
	// Idle
	EnemyFSM.CreateState({ .StateValue = FSM_State_GreenEmpusa::GreenEmpusa_Idle,
	.Start = [=] {
	EnemyRenderer->ChangeAnimation("em0101_fly_orb_absorb_loop.fbx");
	},
	...
	EnemyFSM.ChangeState(FSM_State_GreenEmpusa::GreenEmpusa_Idle);
}
