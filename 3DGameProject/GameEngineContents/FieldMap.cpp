...
std::shared_ptr<FieldMap> FieldMap::CreateFieldMap(GameEngineLevel* _Level, const std::string_view& _FBXName, const float4& _MapPosition, const float4& _MapScale, const float4& _MapRotation)
{
	...
	switch (GameEngineOption::GetOption("Shader")) //메쉬를 깝니다
	{
	case GameEngineOptionValue::Low:
	{
		Result->FieldMapRenderer->SetFBXMesh(_FBXName.data(), "FBX_Low"); 
	}
	break;
	case GameEngineOptionValue::High:
	{
		Result->FieldMapRenderer->SetFBXMesh(_FBXName.data(), "FBX"); 
	}
	break;
	default:
		break;
