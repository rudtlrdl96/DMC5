	...
	// Renderer 생성
	{
		...
		AnimationEvent::LoadAll({ .Dir = NewDir.GetFullPath().c_str(), .Renderer = Renderer, .RendererLocalPos = {0, -75, 0},
			...
			.CallBacks_void = {
				...
				std::bind(&PhysXCapsuleComponent::SetLinearVelocityZero, PhysXCapsule.get()),
				std::bind([=] {MoveCheck = true; }),
				std::bind([=] {DelayCheck = true; }),
				std::bind(&PhysXCapsuleComponent::TurnOnGravity, PhysXCapsule.get()),
		...
		FSM.CreateState({ .StateValue = FSM_State_Nero::Nero_RQ_ComboA_1,
			.Start = [=] {
				...
				if (0 < ExceedLevel)
				{
					...
					TimeEvent.AddEvent(0.05f, [=](GameEngineTimeEvent::TimeEvent& _Event, GameEngineTimeEvent* _Manager) {UseExceed(); });
				}
				else
				{
		...
		FSM.CreateState({ .StateValue = FSM_State_Nero::Nero_RQ_Skill_HR,
			.Start = [=] {
				...
				if (0 < ExceedLevel)
				{
					...
					TimeEvent.AddEvent(0.05f, [=](GameEngineTimeEvent::TimeEvent& _Event, GameEngineTimeEvent* _Manager) {UseExceed(3); });
				}
				else
				{
