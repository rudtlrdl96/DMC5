...
Texture2D BloodVesselTexture : register(t4);
...
DeferredOutPut MeshAniTexture_PS(Output _Input)
{
    ...
    if(0.0f != AtosData.g && 0.1f < _Input.TEXCOORD.y)
    {
        float4 BloodData = BloodVesselTexture.Sample(ENGINEBASE, _Input.TEXCOORD.xy);
        
        AlbmData.rgb += BloodData.rgb * AtosData.g;
        Result.GlamTarget.rgb = BloodData.rgb * AtosData.g * 2.0f;
